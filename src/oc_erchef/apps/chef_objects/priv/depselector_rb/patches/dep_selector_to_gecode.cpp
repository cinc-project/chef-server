...
// Gecode 6.x removed the 'share' parameter from the Space copy constructor
// and from VarArray::update() calls.
#ifdef GECODE_VERSION_6
VersionProblem::VersionProblem(VersionProblem & s)
    : Space(s),
#else
...
#ifdef GECODE_VERSION_6
    package_versions.update(*this, s.package_versions);
    disabled_package_variables.update(*this, s.disabled_package_variables);
    total_disabled.update(*this, s.total_disabled);
    total_required_disabled.update(*this, s.total_required_disabled);
    total_induced_disabled.update(*this, s.total_induced_disabled);
    total_suspicious_disabled.update(*this, s.total_suspicious_disabled);
    at_latest.update(*this, s.at_latest);
    total_preferred_at_latest.update(*this, s.total_preferred_at_latest);
    total_not_preferred_at_latest.update(*this, s.total_not_preferred_at_latest);
#else
...
// Support for gecode
// Gecode 6.x removed the 'share' parameter from copy()
#ifdef GECODE_VERSION_6
Space* VersionProblem::copy()
{
    return new VersionProblem(*this);
}
#else
...
#define INT_VAR_DEGREE_MIN INT_VAR_DEGREE_MIN()
...
int VersionProblem::InnerSolve(VersionProblem * problem, int &itercount, VersionProblem** best_solution)
{
    ...
    *best_solution = NULL;
    ...
    // In Gecode 6.x, RBS template parameter order is <T, Engine>
    // In Gecode 4.x/5.x, RBS template parameter order is <Engine, T>
#ifdef GECODE_VERSION_6
    RBS<VersionProblem, DFS> solver(problem, options);
#else
...
#ifdef GIST_ENABLED
#ifdef GECODE_VERSION_6
    VersionProblem * trial = dynamic_cast<VersionProblem*>(this->copy());
#else
